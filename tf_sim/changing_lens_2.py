...
def main():
    ...
    for ratio in np.arange(0.2, 0.6, 0.05):
        ...
        np.savetxt("output/{ratio:.2f}.csv".format(ratio=np.round(ratio, 2)), horizontally_averaged.numpy()[0, :, 0], fmt="%.4f")
        ...
        tf.io.write_file("output/{ratio:.2f}.png".format(ratio=np.round(ratio, 2)), img)
