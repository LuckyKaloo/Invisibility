...
z_lenticular_list = np.arange(0.7, 0.09, -0.02)
z_camera_list = np.arange(0.878, 0.268, -0.02)

zipped_list = zip(z_lenticular_list, z_camera_list)

for z_lenticular, z_camera in zipped_list:
    ...
    with open('{lpi} lpi, {z_lenticular} cm.csv'.format(lpi=lpi, z_lenticular=z_lenticular), 'a') as f_rays:
        f_rays.truncate(0)
        ...
        for n_run in range(0, NUM_UPDATE):
            ...
            print(
                ('lpi = {lpi}, z_lenticular = {z_lenticular}, ' +
                 ...
                 'avg angles = {average_angles}, total pos = {total_positions}, expected pos = {expected_pos}')
                    ...
                    .format(lpi=lpi, z_lenticular=z_lenticular,
    ...
    with open('{lpi} lpi, {z_lenticular} cm info.csv'.format(lpi=lpi, z_lenticular=z_lenticular), 'w') as f_info:
